...
{
	//
	tick_total = SystemCoreClock/1000/8;   //1ms 经过的ticks数量
	tick_fact = (SystemCoreClock/1000000L + 7) / 8;   //1us 要经过的tick数
	
	SysTick_Config(tick_total);   //设置时钟为1ms中断
	
	//修改一下时钟源
	SysTick_CLKSourceConfig(SysTick_CLKSource_HCLK_Div8);   //8分配
}
